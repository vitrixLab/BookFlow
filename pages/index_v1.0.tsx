    ...
    <>
          ...
          <section className="trust-bar" aria-labelledby="trust-heading">
            ...
            <div className="trust-inner">
              {trustMetrics.map((m, i) => (
                <div className="trust-metric" key={i} data-reveal>
          ...
          <section id="features" className="section" aria-labelledby="features-heading">
            ...
            <div className="features-grid">
              {landing.features.items.map((item, i) => (
                <article className="feature-card" key={i} data-reveal>
          ...
          <section id="how-it-works" className="section section-alt" aria-labelledby="steps-heading">
            ...
            <ol className="steps-list" aria-label="Onboarding steps">
              {landing.steps.items.map((step, i) => (
                <li className="step-item" key={i} data-reveal>
          ...
          <section className="section" aria-labelledby="testimonials-heading">
            ...
            <div className="testimonials-grid">
              {landing.testimonials.items.map((t, i) => (
                <figure className="testimonial-card" key={i} data-reveal>
              ...
              <div className="cta-actions" role="group" aria-label="Sign up actions">
                <button
                  ...
                  style={{ 
                    ...
                    border: 'none',
                    cursor: 'pointer',
                    justifyContent: 'center'         // textAlign: 'center', ← this centers the text
                  }}
                >
                  {landing.cta.button}
      ...
      <style jsx global>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (prefers-reduced-motion: no-preference) { [data-reveal] { opacity: 0; transform: translateY(24px); transition: opacity 0.55s ease, transform 0.55s ease; } [data-reveal].revealed { opacity: 1; transform: none; } }
        @media (prefers-reduced-motion: reduce) { [data-reveal] { opacity: 1; transform: none; } }
      `}</style>

      <style jsx>{`
        :root { --navy: #001e4a; --navy-mid: #00317a; --blue: #0a6ed1; --blue-light: #d6eaff; --accent: #f0a500; --text: #0f1c2e; --muted: #4a6278; --border: rgba(0,0,0,0.09); --surface: #ffffff; --surface-alt: #f4f7fb; --radius: 14px; --shadow: 0 4px 24px rgba(0,30,74,0.10); --shadow-lg: 0 16px 48px rgba(0,30,74,0.16); --font-display: 'Fraunces', Georgia, serif; --font-body: 'Plus Jakarta Sans', system-ui, sans-serif; }
        ...
        .nav-link::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 2px; background: var(--blue); transform: scaleX(0); transform-origin: left; transition: transform 0.2s ease; }
        ...
        .btn { text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.7rem 1.5rem; border-radius: 8px; font-family: var(--font-body); font-weight: 700; font-size: 0.9rem; border: 2px solid transparent; cursor: pointer; transition: all 0.18s ease; min-height: 44px; white-space: nowrap; }
        ...
        .hero { position: relative; overflow: hidden; display: flex; align-items: center; gap: clamp(2rem, 5vw, 6rem); padding: 5rem clamp(1.5rem, 6vw, 5rem) clamp(4rem, 8vw, 8rem); 
            
            max-width: 100%; margin: 2rem auto; 
            background: linear-gradient(120deg, #f0f4ff 0%, #0a6ed1 300%);
        ...
        .feature-card { padding: 2.5rem; border-radius: var(--radius); border: 1px solid var(--border); background: var(--surface); transition: transform 0.25s ease, border-color 0.25s ease, box-shadow 0.25s ease; }
        ...
        .faq-chevron { font-size: 0.9rem; color: var(--muted); flex-shrink: 0; transition: transform 0.2s ease; }
        .faq-toggle[aria-expanded="true"] .faq-chevron { transform: rotate(180deg); }
        .faq-answer { max-height: 0; overflow: hidden; transition: max-height 0.3s ease, padding 0.3s ease; padding-bottom: 0; }
        ...
        .cta-btn {
          background: #0a6ed1;  color: #fff; border-color: #0a6ed1;
          font-size: 1.2rem;
          padding: 1rem 2.8rem;
        ...
        .cta-btn:hover {
          background: #0860a0; border-color: var(--blue);
          transform: translateY(-1px);
          box-shadow: 0 14px 36px rgba(10, 110, 209, 0.2);
