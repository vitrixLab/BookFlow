...
def is_greeting(question):
    """Returns True if the input is a greeting, using whole‑word tokens."""
...
# ----- 6.2 Linguistic noun-phrase extraction (unchanged) -----
...
# ----- 6.4 RAG cache (new) -----
def rag_cache(question, top_k=3):
    """
    Search the BookFlow knowledge binary and return a clean answer.
    Handles chunks that may contain JSON fragments.
    """
    ...
    # Pick the first meaningful chunk
    raw_text = results[0].text
    source = results[0].source

    # Try to extract a clean answer from the chunk
    clean_answer = raw_text.replace('\n', ' ').strip()

    # Attempt to parse as JSON if it looks like JSON
    if raw_text.strip().startswith('{'):
        try:
            data = json.loads(raw_text)
            # If it's a FAQ entry with "q" and "a", return the answer
            if isinstance(data, dict) and 'a' in data:
                clean_answer = data['a']
            # If it's a larger object, try to pick a relevant field
            # Add here if you know other structures
        except json.JSONDecodeError:
            pass   # not valid JSON, keep the cleaned text

    return f"From {source}: {clean_answer}", source
...
def intelligent_cache(question):
    ...
    Tries in order:
      ...
      3. RAG retrieval from BookFlow documentation
      4. Model‑guided probe → exact cache
    """
