...
def extract_head_noun(question):
    ...
    Returns the first noun phrase after 'what is' / 'define' etc., or the last noun phrase.
    """
    ...
    # Simple grammar: chunk noun phrases (determiners, adjectives, nouns)
    ...
    # Return the last (often the most specific) NP, or the first if none
    return nps[-1] if nps else None

# ----- 6.3 Cache lookup (linguistic + model‑guided) -----
...
def intelligent_cache(question):
    """
    Hybrid cache: try linguistic extraction first, then model‑guided prediction.
    Returns (answer, word, method).
    """
    # 1. Linguistic head noun extraction
    head = extract_head_noun(question)
    if head:
        ans = lookup_cache(head)
        if ans:
            return (ans, head, "linguistic")
    # 2. Model‑guided probe
...
# --------------------------------------------------------------------
# 8. Database setup (same as before)
# --------------------------------------------------------------------
