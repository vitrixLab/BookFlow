...
# RAG cache imports
from knowledge_loader import load_knowledge_bin, search_knowledge
KNOWLEDGE_BIN_PATH = 'public/knowledge.bin'   # adjust path if needed
RAG_ENABLED = True                            # toggle for testing
knowledge_chunks = load_knowledge_bin(KNOWLEDGE_BIN_PATH) if RAG_ENABLED else []
...
# ----- 6.1 Improved intent detection (token‑based greetings) -----
def is_greeting(question):
    tokens = word_tokenize(question.lower())
    greeting_words = {"hi", "hello", "hey", "good", "morning", "afternoon", "evening",
                      "how", "are", "you", "what's", "up", "sup", "yo"}
    if tokens[0] in {"hi", "hello", "hey", "sup", "yo", "good"}:
        return True
    if "how" in tokens and "you" in tokens:
        return True
    if len(tokens) >= 2 and tokens[0] == "what" and (tokens[1] == "'s" or tokens[1] == "is") and "up" in tokens:
        return True
    return False
...
# ----- 6.2 Linguistic noun-phrase extraction -----
...
def model_guided_cache(question, temperature=0.2, probe_tokens=5):
    ...
    words = probe_str.split()
    meaningful_words = [w for w in words if len(w) > 1]
    candidates = meaningful_words if meaningful_words else words
    for w in candidates:
...
# ----- 6.4 RAG cache (improved) -----
def rag_cache(question, top_k=3, threshold=0.15):
    """
    Search the BookFlow knowledge binary and return a clean answer.
    Only returns an answer if the top chunk's score is above the threshold.
    """
    if not knowledge_chunks:
        return None, ""
    results = search_knowledge(question, knowledge_chunks, top_k)
    if not results:
        return None, ""

    best_chunk = results[0]
    raw_text = best_chunk.text
    source = best_chunk.source

    # Calculate score (same simple token overlap used in search_knowledge)
    q_tokens = set(re.findall(r'[a-z0-9]+', question.lower()))
    chunk_tokens = set(re.findall(r'[a-z0-9]+', raw_text.lower()))
    score = len(q_tokens.intersection(chunk_tokens)) / max(len(q_tokens), 1) if q_tokens else 0.0
    ...
    # If the chunk is in "Q: ... A: ..." format, extract only the answer part
    if ' A: ' in clean:
        parts = clean.split(' A: ', 1)
        if len(parts) > 1:
            clean = parts[1].strip()
    elif clean.startswith('A:'):
        clean = clean[2:].strip()

    return f"From {source}: {clean}", source
...
def intelligent_cache(question):
    ...
    if RAG_ENABLED:
        ans, source = rag_cache(question)
        if ans:
            return (ans, source, "rag_retrieval")
...
# --------------------------------------------------------------------
# 9. Main chat loop (with Unicode guard)
# --------------------------------------------------------------------
...
while True:
    try:
        ...
        print("Sorry, I only understand plain English letters. Please try again.")
        continue
